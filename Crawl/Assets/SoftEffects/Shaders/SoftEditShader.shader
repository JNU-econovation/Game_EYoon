...
Shader "SoftEffects/SoftEditShader"
{
	Properties
	{
		[PerRendererData]_MainTex("Main Texture", 2D) = "black" {}
		_MainEditTex("Edit Texture", 2D) = "black" {}
		...
		[Toggle(_SHAD_ON)]_UseShadows("Use Shadows", Int) = 0
...
#define _Flip unity_SpriteFlipArray[unity_InstanceID]
		...
		struct v2f
	{
		...
		CGPROGRAM
			...
			{
				half4 color =  (tex2D(_MainEditTex, IN.texcoord)) * IN.color;
				//half4 ct = color;
				//color = color.a;
				//color = half4(1,1,1, color.a);
				//color.rgb = ct.rgb;
